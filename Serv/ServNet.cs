...
public class ServNet
{
    ...
    public HandleLeaderBorder handleLeaderBorder = new HandleLeaderBorder();
    ...
    private void HandleMsg(Conn conn, ProtocolBase protoBase)
    {
        ...
        // 排行榜协议
        else if (name == "LeaderBoard" || name == "InsertLeaderBoard")
        {
            MethodInfo mm = handleLeaderBorder.GetType().GetMethod(methodName);
            if (mm == null)
            {
                string str = "[警告]HandleMsg没有处理排行榜方法 ";
                Console.WriteLine(str + methodName);
                return;
            }
            Object[] obj = new object[] {conn, protoBase};
            Console.WriteLine("[处理获取排行榜请求 " + conn.GetAdress() + " :" + name);
            mm.Invoke(handleLeaderBorder, obj);
        }
        //角色协议分发
        else
        {
